...
export class CallsComponent {
  ...
  dropClicked(time: String) {
    ...
    this.sorted_date.forEach((x: any, i: any) => {
      ...
      call.Recommendation = x["ai_narration"]["Recommendation"];
  ...
  //   this.localDate = localCompleteDate2.substring(0, localCompleteDate2.length - 1);
  //     },1000)
  //   }
    ...
    if (!this.authService.LoginStatus() == true) {
      ...
      this.callService.getCart(dt_call).subscribe(
        (result) => {
          ...
          let date = new Date(this.currentDate.getTime() - 30 * 60 * 1000)
    ...
    this.callService.getCart(cd).subscribe(
      (result) => {
        ...
        this.sorted_date.forEach((x: any, i: any) => {
          ...
          if (x && x["ai_narration"]["ReceptionRating"] && typeof x["ai_narration"]["ReceptionRating"] == 'object') {
            let tot = Object.keys(x["ai_narration"]["ReceptionRating"]).length;
            console.log('total : ', tot)
            let pos = 0;
            let neg = 0;
            let minus_tot = 0
            Object.keys(x["ai_narration"]["ReceptionRating"]).forEach((i) => {
              if (x["ai_narration"]["ReceptionRating"][i] === 'NA' || x["ai_narration"]["ReceptionRating"][i] === 'N/A') {
                minus_tot++
              }
              if (x["ai_narration"]["ReceptionRating"][i] === 'yes' || x["ai_narration"]["ReceptionRating"][i] === "yes") {
                pos++;
              } else {
                neg++;
              }
            ...
            if (this.pos_rating > 60) {
              call.recepitionist_rating = "Good"
            }
            else if (this.pos_rating < 40) {
              call.recepitionist_rating = "Poor"
            }
            else { call.recepitionist_rating = "Normal" }
          }
  ...
  titlePanel: GuiTitlePanel = {
    ...
    template: () => {
      return `
			<div class='title-panel-example' style="font-weight:bold;font-size:large;vertical-align: middle;" >List of Calls</div>
			`;
    },
  };
  ...
  columns: Array<GuiColumn> = [
    ...
    // } 
    // console.log('from grid +++ ', value)
    // return `<div style="color: red;font-weight: bold"><span style='display:inline-block;min-width:60px'>${Object.keys(value)}</span> <span><img src='assets/img/poor.webp' style='max-width:30px'></span></div>`;

    // if (value == "Poor") {
    //   return `<div style="color: red;font-weight: bold"><span style='display:inline-block;min-width:60px'>${typeof value}</span> <span><img src='assets/img/poor.webp' style='max-width:30px'></span></div>`;
    // } else if (value == "Good") {
    //   return `<div style="color: green;font-weight: bold;font-style:italic"><span style='display:inline-block;min-width:60px'>${value}</span> <span><img src='assets/img/good.webp' style='max-width:30px'></span></div>`;
    // } else {
    //   return `<div style="color: grey"><span style='display:inline-block;min-width:60px'>${value}</span> <span><img src='assets/img/ok.webp' style='max-width:30px'></span></div>`;
